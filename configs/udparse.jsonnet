...
local num_bins = std.parseJson(std.extVar("NUM_BINS"));
...

{
    ...
    model: {
        ...
        metrics: {
            ...
            'ece': {
                type: if task == 'deprel' then 'ud-ece' else 'expected-calibration-error',
                num_bins: num_bins
            }
        },
    ...
    trainer: {
        ...
        optimizer: {
            ...
            lr: learning_rate,
            parameter_groups: [
                [['.*trnasformer_model\\.embeddings.*'], {'requires_grad': false}],
